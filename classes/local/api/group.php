...
class group {
     ...
     * Otherwise (group == content::VISIBILITY_GROUP), the user is checked against a
     * group colletion.
     *
     ...
     */
    public static function is_content_group_member($groupmode, $groupid, $groupingid, $userid, $useridtocheck) {
        if (($groupid == content::VISIBILITY_GROUP) || ($groupid < 0)) {
            if ($groupid > 0) {
                ...
                if ($groupmode == VISIBLEGROUPS) {
                    ...
                    $ismember = self::has_same_memberships($groupingid, $userid, $useridtocheck, false);
                } else if ($groupmode == SEPARATEGROUPS) {
                    // Only users in a group with the user can view the slot.
                    $ismember = self::has_same_memberships($groupingid, $userid, $useridtocheck, true);
                }
            } else {
                ...
                $ismember = $ismember = self::has_same_group($groupid, $userid, $useridtocheck);
            }
     ...
     */
    public static function has_same_memberships($groupingid, $userid, $useridtocheck, $strict = false) {
        global $DB;
...
EOF;
     ...
     */
    public static function has_same_group($groupid, $userid, $useridtocheck) {
        global $DB;
...
EOF;
     ...
     * @return int
     */
    public static function has_same_course($courseid, $userid, $useridtocheck) {
        global $DB;
...
EOF;
     ...
     */
    public static function group_list($courseid, $groupingid, $userid, $groupmode = 1) {
        global $DB;
...
EOF;
    ...
    public static function get_name($groupid) {
        global $DB;
