 ...
 */
function studio_api_item_generate_hash($slotid) {
    global $DB;
    $sql = <<<EOF
...
EOF;
    ...
    if ($slot->contenttype > content::TYPE_TEXT && $slot->contenttype < content::TYPE_FOLDER) {
        ...
        if (empty($slot->fileid)) {
            ...
            } else if ($file = $fs->get_file($slot->contextid, 'mod_openstudio', 'notebook', $slot->fileid, '/', $slot->content)) {
                $content = $file->get_contenthash();
            }
        }
    } else {
 ...
 */
function studio_api_item_log($slotid) {
    global $DB;
    $contenthash = studio_api_item_generate_hash($slotid);
    if (!empty($contenthash)) {
        ...
        if ($DB->record_exists('openstudio_content_items', $params)) {
            throw new coding_exception('A slot item already exists for this slot. ' .
                'Move it with studio_api_item_toversion or remove it with studio_api_item_delete'  .
                'before logging a new one.');
        }
        $itemrecord = (object) array(
            'contenthash' => $contenthash,
            'containerid' => $slotid,
            'containertype' => item::CONTENT,
            'timeadded' => time()
        );
 ...
 */
function studio_api_item_toversion($slotid, $versionid) {
    global $DB;
 ...
 * @return array
 */
function studio_api_item_get_occurences($contenthash, $liveonly = true) {
    global $DB;
...
EOF;
        $slotparams = array_merge($params, array('containertype' => STUDIO_SLOT_ITEM_CONTAINER_SLOT));
        $versionsql = <<<EOF
            SELECT i.id, i.containerid, i.containertype, i.timeadded, u.id as userid, u.firstname, u.lastname,
                   u.middlename, u.alternatename, u.firstnamephonetic, u.lastnamephonetic
              FROM {openstudio_content_items} i
          ...
          ORDER BY i.timeadded ASC
EOF;
        $versionparams = array_merge($params, array('containertype' => STUDIO_SLOT_ITEM_CONTAINER_VERSION));
 ...
 */
function studio_api_item_count_occurences($contenthash, $cmid = 0, $liveonly = true) {
    global $DB;
    ...
    if (empty($contenthash)) {
        ...
        $params = array(
            'contenthash' => $contenthash,
            'typecontent' => STUDIO_SLOT_ITEM_CONTAINER_SLOT
        );
        if ($cmid > 0) {
        ...
        if ($liveonly) {
            ...
            $from = '      FROM {openstudio_content_items} ssi '
                    ...
                    .'LEFT JOIN {openstudio_content_versions} sv ON ssi.containerid = sv.id AND ssi.containertype = :typeversion ';
            $where = '    WHERE (ss.id IS NOT NULL AND ss.deletedtime IS NULL '
                    .'       OR sv.id IS NOT NULL AND sv.deletedtime IS NULL) '
                    .'      AND ssi.contenthash = :contenthash ';
            $params['typeversion'] = STUDIO_SLOT_ITEM_CONTAINER_VERSION;
        }
 ...
 * @return bool
 */
function studio_api_item_delete($containerid, $containertype = STUDIO_SLOT_ITEM_CONTAINER_SLOT) {
    global $DB;
