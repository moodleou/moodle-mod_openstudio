...
// Call search plugin check to make sure search lib is included if it's installed.
mod_openstudio\local\util::search_installed();
 ...
 */
function studio_api_search_query($cm, $searchtext,
 ...
 */
function studio_api_search_get_slot_document($cm, $slot, $useidonly = false) {
    // Do nothing if OU search is not installed.
    if (!\mod_openstudio\local\util::search_installed()) {
        return true;
    }
 ...
 */
function studio_api_search_update($cm, $slot) {
    // Do nothing if OU search is not installed.
    if (!\mod_openstudio\local\util::search_installed()) {
        return true;
    }
    ...
    // Get search document.
    $doc = studio_api_search_get_slot_document($cm, $slot);
 ...
 */
function studio_api_search_delete($cm, $slot, $useidonly = false) {
    // Do nothing if OU search is not installed.
    if (!mod_openstudio\local\util::search_installed()) {
        return true;
    }
    ...
    // Get search document.
    $doc = studio_api_search_get_slot_document($cm, $slot, $useidonly);
    if ($doc->find()) {
        return $doc->delete();
    }
