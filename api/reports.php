 ...
 */
function studio_api_reports_get_activity_summary($courseid, $studioid) {
    global $DB;
...
SELECT COUNT(DISTINCT sc.id) as slotaudiocomments
  ...
  JOIN {files} f ON f.itemid = sc.id AND f.component = 'mod_studio'AND f.filearea = 'slotcomment'
 WHERE s.openstudioid = ?
   AND s.deletedby IS NULL
 ...
 */
function studio_api_reports_get_total_slots($studioid) {
    global $DB;
 ...
 */
function studio_api_reports_get_total_flags($studioid) {
    global $DB;
...
EOF;
    ...
    $slotstop20 = $DB->get_recordset_sql($sql, array($studioid, STUDIO_PARTICPATION_FLAG_READ_SLOT), 0, 20);
...
EOF;
    ...
    $slotstrackedtop20 = $DB->get_recordset_sql($sql, array($studioid, STUDIO_PARTICPATION_FLAG_FOLLOW_SLOT), 0, 20);
 ...
 */
function studio_api_reports_get_total_storage($studioid) {
    global $DB;
  ...
  SELECT s.mimetype,
    ...
    FROM {openstudio_contents} s
    JOIN {files} f ON f.itemid = s.fileid AND f.component = 'mod_studio'
   WHERE s.openstudioid = ?
...
EOF;
  ...
  SELECT SUM(f.filesize) as storage
    FROM {openstudio_content_versions} sv
    JOIN {openstudio_contents} s ON s.id = sv.contentid
    JOIN {files} f ON f.itemid = sv.fileid AND f.component = 'mod_studio'
                                           AND f.filearea = 'slot'
   WHERE s.openstudioid = ?
...
EOF;
  ...
  SELECT SUM(f.filesize) as storage
    ...
    JOIN {files} f ON f.itemid = c.id AND f.component = 'mod_studio'
                                      AND f.filearea = 'slotcomment'
   WHERE s.openstudioid = ?
 ...
 */
function studio_api_reports_get_activity_log($courseid, $cmid) {
    global $DB;
