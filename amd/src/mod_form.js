...
const completionFormMinMax = (formElement) => {
    const forumngCompletionRequire = formElement.querySelectorAll('.openstudio-completion-require:checked');
    if (!forumngCompletionRequire.length) {
        toggleHiddenClassWordcount(true);
    }

    formElement.querySelectorAll('input.openstudio-completion-require').forEach(item => {
